...
const TimeInputField = ({ mode, field,fieldIndex, deleteField, documentFields,setDocumentFields }) => {
    ...
    return <div className={mode !== 'preview' ? 'document-field-wrapper' : ''}>
        {mode === 'preview' && <div style={{textAlign : field.alignment, width : '100%', background: 'transparent', display : 'grid', gridTemplateColumns: 'auto 10px auto'}}>
            ...
            <span>{field.label}</span><span/><input type="time" onChange={e=>updateField(e,'value')} value={field.value}/>
        </div>}
        {mode !== 'preview' && <div>
            ...
            <div style={{textAlign : field.alignment, width : '100%', background: 'transparent', display: 'grid', gridTemplateColumns : 'auto 10px auto'}}>
                ...
                <span/>
                <input type="time" onChange={e=>updateField(e,'value')} value={field.value}/>
                
            </div>
            <br/>
            Label : <input type="text" value={field.label} placeholder="Time Field Label" onChange={e=>updateField(e,'label')}/><br/>
        </div>}
    </div>
}
